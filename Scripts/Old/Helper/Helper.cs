...
{  
    ...
    public static Vector3 GetDegreesMode(Vector3 degree)
    {
        Vector3 modDegree = new Vector3(GetMod360(degree.x), GetMod360(degree.y), GetMod360(degree.z));

        return modDegree;
        float GetMod360(float n) => n % 360;
    }

    public static int GetEnemyLayer() => LayerMask.GetMask("Enemy");
    public static int GetNPCLayer() => LayerMask.GetMask("NPC");
    public static int GetEnemyAndNPCLayer() => LayerMask.GetMask("Enemy", "NPC");
}
